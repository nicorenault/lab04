...
Publicacion::Publicacion(DTFecha* fecha, TipoPublicacion tipo, std::string texto, float precio, bool activa) {
...
bool Publicacion::cumpleFiltro(TipoPublicacion tpub, float precioMin, float precioMax, TipoInmueble tipoInm) const {
    ...
    if (tipoInm != Todos) {
        ...
        if ((tipoInm == TCasa) && (!in->esCasa()))   return false;
        if ((tipoInm == TApartamento) && (in->esCasa())) return false;
    }
    return true;
}
