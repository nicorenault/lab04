...
void altaPublicacion(){
    ...
    for (itInmueble = inmuebles.begin(); itInmueble != inmuebles.end(); ++itInmueble) {
        std::cout << "- Código: " << itInmueble->getCodigo()
            ...
            << ", Fecha: " << itInmueble->getFechaComienzo()->toString() << std::endl;
    }
    int codigoInmueble;
    std::cout << "Inmueble: ";
...
void suscribirseNotificaciones(){
    ...
    std::cout << "Lista de Propietarios y Clientes:\n";
    std::set<DTUsuario> propietarios = controlador->listarPropietarios();
    std::set<DTUsuario>::iterator it;
    for (it = propietarios.begin(); it != propietarios.end(); ++it) {
        std::cout << "- Nickname: " << it->getNickname() << ", Nombre: " << it->getNombre() << std::endl;
    }
    std::set<DTUsuario> clientes = controlador->listarClientes();
    for (it = clientes.begin(); it != clientes.end(); ++it) {
        std::cout << "- Nickname: " << it->getNickname() << ", Nombre: " << it->getNombre() << std::endl;
    }

    std::cout << "Nickname de cliente o propietario que desea suscribirse: ";
    ...
    if (disponibles.empty()) {
        ...
        salir = 0;
            ...
            std::cin >> salir;
            std::cin.ignore();
            std::cout << "Suscripcion registradas correctamente.\n\n";
        }
